...
from my_model.tools.ai.torch_utils import resize_for_tensors
...
class Classifier(Backbone):
    def __init__(self, model_name='resnet50', num_classes=2, mode='fix', fmr=None):
    ...
    def forward(self, x, with_cam=True, separate=False, with_fmr=False):
        N, C, H, W = x.size()
        ...
        if with_cam:
            ...
            CAM = F.relu(CAM)
            if with_fmr:
                assert self.fmr is not None, "with_fmr=True but self.fmr is None"
                # 在高维特征 x 上做恢复/门控融合
                x_f3, x_f2, m = self.fmr(x)  # x_f3/x_f2: (B,2048,16,16), m:(B,1,16,16)

                features_f3 = self.classifier(x_f3)  # (B,num_classes,16,16)
                logits_f3 = self.global_average_pooling_2d(features_f3)
                return logits, features, CAM, x, x_f3, x_f2, m, logits_f3, features_f3
...
class AffinityNet(Backbone):
    ...
    def get_edge(self, x, image_size=256, stride=4):  # todo
