...
from my_model.core.aff_utils import *

from my_model.tools.ai.augment_utils import *
from my_model.tools.ai.torch_utils import one_hot_embedding

from my_model.tools.general.xml_utils import read_xml
from my_model.tools.general.json_utils import read_json
from my_model.tools.dataset.voc_utils import get_color_map_dic
...
class VOC_Dataset(torch.utils.data.Dataset):
    def __init__(self, root_dir, domain, with_id=False, with_tags=False, with_mask=False):
        ...
        self.image_dir = self.root_dir + 'tif/'
        self.xml_dir = self.root_dir + 'Annotations/'
        self.mask_dir = self.root_dir + 'label/'##
        # self.mask_dir = self.root_dir + 'SegmentationClass/'
        self.image_id_list = [image_id.strip() for image_id in open(r'waste2/{}.txt'.format(domain)).readlines()]  # todo
    ...
    def get_image(self, image_id):
        ...
        image = imageio.imread(self.image_dir + image_id)[:,:,:3]
        # image = imageio.imread(self.image_dir + image_id + '.tif')##
        return image

    def get_mask(self, image_id):
        mask_path = self.mask_dir + image_id  # todo
        # print(mask_path)
    ...
    def get_tags(self, image_id):
        ...
        if self.domain == 'train':
            d = np.load(r'waste2/cls_labels_train.npy', allow_pickle=True).item()  # todo
        elif self.domain == 'test':
            d = np.load(r'waste2/cls_labels_test.npy', allow_pickle=True).item()  # todo
        else:
            d = np.load(r'waste2/cls_labels_val.npy', allow_pickle=True).item()  # todo
        # tags = d[image_id][1:]
        tags = d[image_id]
        return tags
...
class VOC_Dataset_For_Classification(VOC_Dataset):
    def __init__(self, root_dir, domain, transform=None):
        super().__init__(root_dir, domain, with_tags=True)  # todo
        self.transform = transform

        data = read_json('data/waste2.json')
...
class VOC_Dataset_For_Affinity(VOC_Dataset):
    ...
    def __getitem__(self, idx):
        ...
        label = imageio.imread(self.label_dir + image_id)
        # label[label == 2] = 1  # todo
        # label[label == 255] = 1  # todo
